...
app.get('/unresolvedMainReq', async (req, res) => {
    try {
        const unresolvedRequests = await MaintenanceRequest.find({ 'isresolved.status': false })
            .populate('tenantId', 'fname lname HouseNum -_id')
            .exec();

        res.json(unresolvedRequests.map(requestItem => ({
            ...requestItem._doc,
            availableDates: requestItem.availableDates.map(date => ({
                date: date.date, 
...
app.get('/api/Booking/security', async (req, res) => {
    try {
...
app.put('/api/Booking/:bookingId/:actionType', async (req, res) => {
    try {
